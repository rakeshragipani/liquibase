...
do
	echo "$line"
	bash $JENKINS_HOME/liquibase/liquibase --changeLogFile=$line --url=jdbc:postgresql://$hostname:$portnumber/liquibase --username=$lquser --password=$lqpassword --driver=org.postgresql.Driver --classpath=$JENKINS_HOME/liquibase/postgresql-42.2.16.jar update
